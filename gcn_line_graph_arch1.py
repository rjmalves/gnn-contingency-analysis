...
combinations = list(product(TOLS, DROPOUTS, EMBEDDING_D, TRAIN_SPLIT))
...
for c in combinations:
    (tol_inf, tol_sup), dropout, embedding_d, train_split = c
    print(f"Params = {c}")
    for i in range(1, N_EVALS + 1):
        print(f"Eval {i}")
        name = (
            f"k{k}_ti{tol_inf}_ts{tol_sup}_emb{embedding_d}"
            + f"split{train_split}_drop{dropout}_it{i}"
        )
