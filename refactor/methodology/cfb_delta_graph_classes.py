...
# Testes for all graph with N nodes
nos = 7
grafos = nx.read_graph6(f"data/{nos}nodes2c.g6")
print("Total de grafos:", len(grafos))
indices = list(range(len(grafos)))
max_deltas = []
for i, g in zip(indices, grafos):
    print(f"N = {i}")
    G = g
    net = Network(f"{i}", G)
    k = 1
...
plt.scatter(indices, max_deltas)
plt.show()

grafo_max = grafos[np.argmax(max_deltas)]
nx.draw_networkx(grafo_max)
plt.show()
net = Network(f"{i}", grafo_max)
screener = ExhaustiveScreener(net)
deltas = screener.global_deltas(k)
deltas

nx.current_flow_betweenness_centrality(nx.cycle_graph(15))

classes = {k: 0 for k in deltas.keys()}
max_delta = max(list(deltas.values()))
for d in deltas.keys():
    if classes[d] / max_delta >= 0.7:
        classes[d] = 1
    else:
        classes[d] = 0

deltas = np.array(list(deltas.values()))
plt.hist(deltas)
plt.show()
max_delta
