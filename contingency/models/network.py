...
import numpy as np
from networkx.readwrite import read_edgelist, read_graph6
...
class Network:
    ...
    def edge_mapping(self) -> Dict[Tuple[int, int], int]:
        def __eval_edge_mapping() -> Dict[Tuple[int, int], int]:
            ...
            return {
                (self.node_mapping[n[0]], self.node_mapping[n[1]]): i
    ...
    def reverse_edge_mapping(self) -> Dict[int, Tuple[str, str]]:
        if self.__reverse_edge_mapping is None:
            ...
            self.__reverse_edge_mapping = {
                i: (self.node_from_mapping(e[0]), self.node_from_mapping(e[1]))
                for i, e in m.items()
            }
    ...
    def __eval_valid_contingency(
        self,
        edges_indices_to_remove: List[int],
    ) -> bool:
        edges_to_remove = [
            self.edge_from_mapping(i) for i in edges_indices_to_remove
        ]
    ...
    def __eval_contingencies(self, order: int):
        ...
        validities = [
            self.__eval_valid_contingency(e) for e in edge_combinations
        ]
        valids = [i for i, v in enumerate(validities) if v]
        islandings = [i for i, v in enumerate(validities) if not v]

        valid_matrix = np.zeros((len(valids), order), dtype=np.int32)
        island_matrix = np.zeros((len(islandings), order), dtype=np.int32)
    ...
    def valid_contingencies(
        ...
        for i in range(mat.shape[0]):
            contingency = [self.reverse_edge_mapping[e] for e in mat[i, :]]
            yield contingency

    def islanding_contingencies(
        self, order: int
    ) -> Iterator[List[Tuple[str, str]]]:
