...
def eval_delta_contingency(
    graph: nx.Graph, contingency: tuple, reference: Dict[str, float]
) -> float:
...
class Screener:
    ...
    @abstractmethod
    def global_deltas(self, order: int) -> Dict[Tuple[str, str], float]:
        pass

    @abstractmethod
    def normalized_global_deltas(self, order) -> Dict[Tuple[str, str], float]:
        pass


class ExhaustiveScreener(Screener):
    def __init__(self, network: Network, num_processors: int = 1):
        super().__init__(network)
        self.__num_processors = num_processors
        self.__deltas: Dict[int, Dict[tuple, float]] = {}
        self.__global_deltas: Dict[int, Dict[tuple, float]] = {}
    ...
    def __eval_deltas(self, order: int):
        ...
        with Pool(processes=self.__num_processors) as pool:
            ...
            async_res = {
                tuple(c): pool.apply_async(f, (g, c, ref))
                for c in self.network.valid_contingencies(order)
            }
            deltas = {i: r.get(timeout=30) for i, r in async_res.items()}
