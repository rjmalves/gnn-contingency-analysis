...
def generate_labels(
    deltas: Dict[tuple, float], tol_sup: float, tol_inf: float
) -> Dict[tuple, int]:
    ...
    for d in deltas.keys():
        if deltas[d] / max_delta >= tol_sup:
            classes[d] = 1
        # elif deltas[d] / max_delta <= tol_inf:
        #     classes[d] = 2
        else:
            classes[d] = 0
    return classes
...
def divide_nodes_in_classes(classes: Dict[tuple, int]) -> Dict[int, np.ndarray]:
...
class GCN(torch.nn.Module):
    def __init__(
        ...
        self.conv4 = GCNConv(hidden_channels, num_outputs)
...
combinations = list(product(TOLS, TRAIN_SPLIT, HIDDEN_CHANNELS))
...
for c in combinations:
    (tol_inf, tol_sup), train_split, channels = c
    print(f"Params = {c}")
    for i in range(1, N_EVALS + 1):
        print(f"Eval {i}")
        name = (
            f"k{k}_ti{tol_inf}_ts{tol_sup}"
            + f"split{train_split}_channels{channels}_it{i}"
        )
        classes = generate_labels(deltas, tol_sup, tol_inf)
        Gl = nx.line_graph(G)
        ...
        data = create_torch_data(
            Gl, train_nodes, test_nodes, nodes_classes, EMBEDDING_D[0]
        )
...
result.to_csv(f"result_k{k}.csv")
